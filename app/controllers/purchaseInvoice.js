...
function getSuppliersInvoices(req, res) {
    ...
    if (profile === "Admin") {
        ...
        purchaseInvoice.find().populate({
            path: 'Supplier', model: 'Supplier', match: { Company: ObjectID(company) },
        ...
        purchaseInvoice.find({ User: id }).populate({
            path: 'Supplier', model: 'Supplier', match: { Company: company },
            populate: { path: 'SupplierType', model: 'SupplierType' }
        }).sort({ CodInvoice: -1 })
            .then(facturas => {
                if (!facturas) {
...
function getInvoiceDetails(req, res) {
    ...
    purchaseInvoiceDetail.find({ PurchaseInvoice: invoiceId }).populate({
...
function getInvoiceSupplierExport(req, res) {
    purchaseInvoiceDetail.find()
        .populate({
            path: 'PurchaseInvoice', model: 'PurchaseInvoice',
