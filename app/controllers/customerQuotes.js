...
function getCustomerQuote(req, res) {
    ...
    if (profile === "Admin") {
        CustomerQuote.find().populate({
            path: 'Customer', model: 'Customer', populate: { path: 'Discount', model: 'Discount' },
...
async function createCustomerQuote(req, res) {
    ...
    let codigoQuote = await CustomerQuote.findOne({ Company: companyId })
        .populate({
            path: 'Customer', model: 'Customer', populate: { path: 'Company', model: 'Company', match: { _id: companyId } },
...
function getCustomerQuotesDetails(req, res) {
    ...
    QuoteDetails.find({ CustomerQuote: customerQuoteId }).populate({
...
function getCustomerAllQuotesDetails(req, res) { //pendiente de arreglar de nuevo para el front
...
async function ImprimirCotizacionHtmlPDF(req, res) {
    ...
    contenidoHtml = contenidoHtml.replace("{{nombreusEmpresa}}", cotizacion.User.Name + " " + cotizacion.User.LastName);
    contenidoHtml = contenidoHtml.replace("{{para}}", cotizacion.AddressedTo ? cotizacion.AddressedTo : cotizacion.Customer.Name);
    ...
    var options = {
        ...
        footer: {
            ...
            contents: {
                ...
                default: " <div class='footer' style='text-align:center; font-size: 7px;'> Gracias por la preferencia, " + cotizacion.Customer.Name +
