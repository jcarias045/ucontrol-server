...
async function getSaleOrders(req, res) {
    ...
    if (profile === "Admin") {  //perfil administrador de la compañia
        saleOrder.find().populate({
            path: 'Customer', model: 'Customer', match: { Company: company },
        ...
        saleOrder.find({ User: id }).populate({
            path: 'Customer', model: 'Customer', match: { Company: company },
            populate: { path: 'Discount', model: 'Discount' }
        }).sort({ CodSaleOrder: -1 })
            .then(order => {
...
function getCustomerQuoteDetails(req, res) { //funcion para obtener los detalles de una cotizacion seleccionada 
    ...
    customerQuotesDetails.find({ CustomerQuote: quoteId }).populate({
...
function getAllSaleOrderDetails(req, res) {
    //cambiando el filtrado para que se haga la exportacion de ordendes de venta a traves
    //del id de la compañia
    ...
    // saleOrderDetails.find().populate({
    //     path: 'Inventory', model: 'Inventory',
    //     populate: ({ path: 'Bodega', model: 'Bodega', match: { Name: 'Principal' } }),
    //     populate: ({ path: 'Product', model: 'Product', populate: { path: 'Measure', model: 'Measure' } })
    // })
